# Handover
**SESSION CLOSED 2026-07-01 — Immediate next action: BU Lead + AM review CLIENT_PROPOSAL_PLENNEGY_V5.docx before any further AI work. OAR-A01 (B-BBEE) expires 2026-07-31 URGENT. See `00_Governance/NEXT_WORK_PACKAGE.md` and `00_Governance/SESSION_CLOSEOUT_2026-06-30.md`.**

**Last updated:** 2026-06-30 — **PF2-010 (V5) COMPLETE — PROPOSAL V5 NARRATIVE & STYLE ENGINE OPERATIONAL.** `proposal_v5_engine.py` v1.0. V5 changes the Proposal Factory from a document assembler into a professional proposal author. The writing standard: every section answers four questions (customer challenge / APPSolve proposal / why this approach / business outcome). All 11 sections fully authored — 0 pending sections, 0 placeholders. Key improvements: Exec Summary authored (Plennegy-specific, "at an inflection point" opening); Understanding authored (specific problem — "each entity manages its own HR records"); Proposed Solution restructured from module catalogue to story (Today → Future → Oracle enables → APPSolve delivers → Outcome); Why APPSolve rewritten from company profile to trust argument; Commercial Position rewritten from legal to collaborative; Next Steps rewritten as joint initiation. Two outputs: `CLIENT_PROPOSAL_PLENNEGY_V5.docx` (48 KB / 11 sections) + `CLIENT_PROPOSAL_PLENNEGY_V5.md` (10 KB — new output type). Style guide: TD-027 PROPOSAL_STYLE_GUIDE.md. Readiness: V.NEXT 72/100 → V5 91/100. GOV-PAE-001: OUM = 0. Regression: 6/6 PASS. **Platform L6.4 → L6.5.**

*Previous: 2026-06-30 — **PF2-009 (PNE) COMPLETE — PROPOSAL NARRATIVE ENGINE OPERATIONAL.** `proposal_narrative_engine.py` v1.0 transforms V2 rendered Markdown into a concise, authored, customer-facing proposal narrative (25–35 pages target). Core rule: Knowledge assets are source material — not proposal sections. Business-outcome framework (outcome / APPSolve value / delivery confidence / customer benefit) applied to every section. 11-section client structure: 9 sections authored by PNE, 2 PENDING (BU Lead + AM). Three outputs: `CLIENT_PROPOSAL_[Tender]_VNEXT.docx` (45 KB Plennegy) + `INTERNAL_REVIEW_PACK_[Tender]_VNEXT.docx` (429 KB — Action Register + Part 1 authored + Part 2 all 41 V2 KB) + `TRACEABILITY_REPORT_[Tender]_VNEXT.docx` (39 KB — authoring trace + compression table). Content compression: HCM Capability 555 segs→6 pages (11×); Methodology 140 segs→4 pages (4.5×); Governance 140 segs→2 pages (9×); Assumptions 497 segs→2 pages (31×). **GOV-PAE-001 enforced** throughout — Oracle Modern Best Practices / Oracle Success Navigator; OUM does not appear anywhere in client output. Standards: TD-026 PROPOSAL_NARRATIVE_AUTHORING_STANDARD.md. CLI: `python3 proposal_narrative_engine.py --tender PLENNEGY-HCM-001`. Regression: 6/6 PASS. **Platform L6.3 → L6.4.**

*Previous: 2026-06-30 — **PF2-010 COMPLETE — PROPOSAL PUBLISHING POLICY ENGINE (PPPE) OPERATIONAL.** `proposal_publishing_policy.py` v1.0 sits between the Renderer and the DOCX layer. It is an architectural enforcement layer — NOT a cosmetic change. 10 deterministic publishing rules applied: R1 (governance subsections removed: 64) + R2 (internal IDs stripped) + R3 (annotation history removed) + R4 (BU decision tables) + R5 (assumption libraries → executive summaries: 10 packs) + **R6 (OUM → Oracle's Modern Best Practices: 48 replacements across all KB-sourced sections)** + R7 (roadmap condensed) + R8 (customer next steps) + R9 (evidence columns stripped) + R10 (page budget per section). Three publication profiles: PROFILE_CUSTOMER (94 KB / 33 sections) + PROFILE_INTERNAL (421 KB / 41 V2 sections) + PROFILE_REVIEW (93 KB / policy annotations). Architecture docs: TD-023 PROPOSAL_PUBLISHING_STANDARD.md + TD-024 CUSTOMER_PROPOSAL_POLICY.md + TD-025 INTERNAL_REVIEW_POLICY.md. CLI: `python3 proposal_publishing_policy.py --tender PLENNEGY-HCM-001`. Regression: 7/7 PASS, 0 violations. **Platform L6.2 → L6.3.** PPPE is complementary to PAE — both read V2; PAE authors from scratch, PPPE filters/cleans KB content.*

*Previous: 2026-06-30 — **PF2-009 COMPLETE — PROPOSAL AUTHORING ENGINE (PAE) OPERATIONAL.** `proposal_authoring_engine.py` v1.0 is the final publishing step of the Proposal Factory. PAE **authors** proposals from scratch — KB assets are source evidence, not proposal sections. Two outputs per tender: `CLIENT_PROPOSAL_[Tender].docx` (executive narrative, 9 authored sections, 2 PENDING, 2 appendices — 61 KB Plennegy) and `INTERNAL_REVIEW_PACK_[Tender].docx` (complete internal document with Action Register + all V2 KB content — 431 KB Plennegy). **PERMANENT PLATFORM RULE GOV-PAE-001 encoded:** Oracle Fusion Cloud proposals must use "Oracle's Modern Best Practices" / "Oracle Success Navigator" — OUM is prohibited for Fusion Cloud. Architecture documents: TD-020 PROPOSAL_AUTHORING_STANDARD.md + TD-021 CLIENT_PROPOSAL_STANDARD.md + TD-022 INTERNAL_REVIEW_PACK_STANDARD.md. CLI: `python3 proposal_authoring_engine.py --tender PLENNEGY-HCM-001`. Regression: 6/6 PASS, 0 violations. **Platform L6.1 → L6.2.** Full Proposal Factory pipeline: KRPE → KVE → ARE → RSE → PSAE → Renderer → PPE (V2) → Shaper (V3) → PEE (V4) → **PAE (CLIENT)**.

*Previous: 2026-06-30 — **PF2-008 COMPLETE — PROPOSAL EDITORIAL ENGINE (PEE) OPERATIONAL.** `pee.py` v1.0 applies 5 editorial transformations to V3 structured proposal → V4 client-ready output. Pipeline: CF (content filter) → BP (boilerplate strip: 8 Oracle marketing patterns) → DD (cross-section dedup: MD5 hash) → BG (page budget enforcement: per-section segment cap) → AU (authored overlays: 6 new sections). V4 structure: 11 body + 5 appendices (A–E). New V4 sections: Proposed Oracle HCM Solution (Sec 3), Implementation Roadmap (Sec 7), Key Risks (Sec 9), HCM Capability Summary (App A), Reference Projects (App D), Glossary (App E). Plennegy V4: CLIENT 94 KB / INTERNAL 97 KB / TRACEABILITY 40 KB / Editorial Report 5 KB. Readiness 4.1/5 → 4.5/5. Architecture docs: EDITORIAL_RULE_LIBRARY.md (33 rules) + PROPOSAL_EDITORIAL_STANDARD.md. CLI: `python3 pee.py --tender PLENNEGY-HCM-001`. V2+V3 regression PASS. **Platform L6.0 → L6.1.** Full pipeline COMPLETE: KRPE → KVE → ARE → RSE → PSAE → Renderer → PPE (V2) → Shaper (V3) → PEE (V4).

*Previous: 2026-06-30 — **PF2-008 COMPLETE — PROPOSAL SHAPING LAYER OPERATIONAL.** `proposal_shaper.py` v1.0 transforms the V2 rendered Markdown into a structured V3 client-ready proposal. Five assembly modes: passthrough, full, full_multi, authored, authored_with_excerpt. V3 structure: 10 body sections + 3 appendices. Key transformations: 7 credential sections consolidated into Sec 4 (Why APPSolve); 3 capability sections moved to Appendix A; 4 authored sections (Governance, Assumptions Summary, Commercial, Next Steps). Plennegy V3 outputs: CLIENT 307 KB / INTERNAL 309 KB / TRACEABILITY 40 KB / V2→V3 Review Report 5 KB. Readiness 2.0/5 → 4.1/5. CLI: `python3 proposal_shaper.py --tender PLENNEGY-HCM-001`. Architecture docs: proposal_shaper.py. PPE (V2) pipeline unaffected — regression PASS. **Platform maturity L5.9 → L6.0.**

*Previous: 2026-06-30 — **PF2-007 COMPLETE — PROPOSAL PUBLISHING ENGINE OPERATIONAL.** `ppe.py` v1.0 consumes the rendered Proposal Factory Markdown + Section Manifest YAML and produces 4 audience-specific DOCX outputs. Three publication profiles: CLIENT (clean client submission — 19 rendered sections, 18 PH + 3 AI omitted, no internal metadata), INTERNAL REVIEW (all 40 sections with ✓/⚠/✏ status badges + completeness matrix), TRACEABILITY (all sections with manifest traceability panels). Plus Review Report (10-criteria quality review, 65/100 CLIENT DRAFT READY). Section classifier: PLACEHOLDER comment authoritative over body content; AI sentinel paragraph excluded from `has_body`. Imports rendering infrastructure from `docx_renderer.py` (PF2-004A). CLI: `python3 ppe.py --tender PLENNEGY-HCM-001`. Plennegy outputs: CLIENT 410 KB / INTERNAL 422 KB / TRACEABILITY 423 KB / REVIEW 44 KB — all validated. Architecture docs: PUBLISHING_ENGINE.md + PUBLICATION_PROFILE_STANDARD.md. **Platform maturity L5.8 → L5.9.** Proposal Factory pipeline now COMPLETE end-to-end: KRPE → KVE → ARE → RSE → PSAE → Renderer → PPE. Open human actions: OAR-A01 (B-BBEE 2026-07-31 URGENT), OAR-C01/C02/C04 (commercial inputs).

*Previous: 2026-06-30 — **PF2-004A COMPLETE — DOCX RENDERER OPERATIONAL.** `docx_renderer.py` v1.0 builds professional Microsoft Word proposals from Proposal Factory Markdown output. Plennegy pilot: `PLENNEGY_PROPOSAL_DRAFT_V1.docx` (422 KB / 4,259 paragraphs / 332 tables / 40 sections) + `PLENNEGY_PROPOSAL_REVIEW_REPORT.docx` (45 KB / 10-criteria quality review). Review mode shows PLACEHOLDER (red), AI-DRAFT (yellow), HUMAN REVIEW (blue) callout boxes. APPSolve brand styles: Calibri, navy headings, table headers, page numbers, headers/footers, auto-TOC field, cover page. CLI: `python3 docx_renderer.py --tender PLENNEGY-HCM-001`. 6/6 regression PASS (markdown pipeline unchanged). **Platform maturity L5.7 → L5.8.** Review report assesses 10 criteria; overall proposal readiness 62/100 (Draft Review Stage); factory performance 78/100 (correct — remaining gaps are human commercial inputs).

*Previous: 2026-06-30 — **PF2-006 COMPLETE — FIRST LIVE OPERATIONAL VALIDATION (PLENNEGY BOM).** Plennegy Oracle HCM Full Suite tender (PLENNEGY-HCM-001) processed end-to-end through Proposal Factory. **2 Platform Defects found and fixed:** C-PSAE-001 (added `--context YAML_FILE` to PSAE CLI — production tenders now loadable from YAML); C-RENDERER-001 (S-49/S-51/A-01 changed from DIRECT to MERGE — assumption packs 2–N were silently dropped). Plennegy render: RENDERED=21/PH=18/AI=3/NF=0 / 11,066 lines / 5 assumption packs in schedule. G-001 applied correctly (Workforce Compensation excluded — Agribusiness ≠ Mining). 6/6 regression PASS after all fixes. **Platform maturity L5.6 → L5.7.** Operational Readiness Score 78/100 (CONDITIONALLY READY). Open human actions: OAR-C01 (HB AM approval), OAR-C02 (Costing), OAR-A01 (B-BBEE URGENT 2026-07-31). Outputs: `09_Active_Tenders/Plennegy/` (Tender Profile, Operational Validation Report, Gap Register, Platform Improvement Register); `Proposals/PLENNEGY-HCM-001/` (rendered proposal + audit).

*Previous: 2026-06-30 — PF2-005 COMPLETE — ASSUMPTION LIBRARY MATERIALISATION COMPLETE. `proposal_renderer.py` AssetIndex extended to scan `08_Commercial/Assumptions/` and index by `document_id` from YAML frontmatter. BeBanking ASP `document_id` defect fixed. AssetIndex: 76 files / 125 IDs. Full pipeline verified: KRPE → KVE (KHS 91/100) → PSAE (6/6 PASS) → Renderer (6/6 PASS). **NOT_FOUND=0 across all 6 regression tenders.** ARM-IT045: RENDERED=25 / PLACEHOLDER=17 / AI-DRAFT=3 / NOT_FOUND=0 (was 16/17/3/9). ARM-IT045 rendered output: 7,347 lines (was 3,537). **Platform maturity L5.5 → L5.6.** NEXT: Plennegy live tender assembly OR next Proposal Factory work package.

*Previous: 2026-06-29 — PF2-004 COMPLETE — MARKDOWN PROPOSAL RENDERER OPERATIONAL. Renderer v1.0 (`proposal_renderer.py`): AssetIndex (50 files / 99 IDs); deterministic Markdown proposal per tender; frontmatter+internal-section stripping; heading demotion (body headings −2 levels); governance flags visible in document header; 6/6 regression PASS (ARM-IT045 full render 3,537 lines + 5 smoke tests); per-section render audit (82-row traceability table). ARM-IT045: RENDERED=16 / PLACEHOLDER=17 / AI-DRAFT=3 / NOT_FOUND=9 (ASP packs not yet materialised). Platform maturity L5.4 → L5.5.*

*Previous: 2026-06-29 — PF2-003 COMPLETE — PROPOSAL SECTION ASSEMBLY ENGINE OPERATIONAL. PSAE v1.0 (`proposal_section_engine.py`): 82 sections; deterministic section manifest per tender; 22/22 self-tests PASS; 6 regression scenarios (ARM-IT045 EBS-AMS→43 mand, HCM-Mining→41 mand, OIC→41 mand, ERP→42 mand, Acumatica→41 mand, BeBanking→40 mand); 0 violations; SI-001/005/006/007 enforced; GOV-010/ADR-001/B-BBEE encoded; manifests written for all 6 regression tenders. Platform maturity L5.3 → L5.4.*

*Previous: 2026-06-29 — PF2-002 COMPLETE — ASSEMBLY RULE ENRICHMENT ENGINE OPERATIONAL. ARE v1.0 (`are.py`): 49 CAP + 13 ASP enriched; 20/20 PASS; 62/62 valid; 6 regression 0 violations; GOV-010 encoded. Registry BUILD-20260629-ARE-001. Platform maturity L5.2 → L5.3.*

*Previous: 2026-06-29 — PF2-001 COMPLETE — RISK SELECTION ENGINE OPERATIONAL. RSE v1.0 (`rse.py`): reads ENTERPRISE_RISK_REGISTER_V1.md; evaluates all 40 RSK assets via AREL V1.0; 17/17 self-tests PASS; 5 regression scenarios (ARM-IT045→14 sel, OracleHCM-P3→15 sel, OIC-P7→6 sel, Acumatica-P11→4 sel, BeBanking-P13→1 sel); 0 validation violations; deterministic. AREL normalization: ERR syntax (CONTAINS/=) normalized at evaluation. 34-field RSEContext (20 KVE + 14 RSE extension). Platform maturity L5.1 → L5.2.*

*Previous: 2026-06-28 — WP19D COMPLETE — KNOWLEDGE PLATFORM VERSION 1.0 COMPLETE. KVE v2.0 (Phase B): 76 validation rules; AREL V1.0 recursive-descent evaluator (86/86 tests PASS); Mode 1 Platform Health (KHS 91/100 — EXCELLENT); KNOWLEDGE_HEALTH_REPORT generated. 5 regression scenarios — all 76 rules, deterministic. VALID_PLATFORMS corrected to canonical 6-value set. KVE is the sole governance gateway. Platform maturity L5.0 → L5.1.*
